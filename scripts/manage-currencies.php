...
// Locate the project root so XML files end up next to the site sources when run locally.
function findProjectRoot(string $startDir, array $markers, int $maxDepth = 6): string
{
    $dir = $startDir;
...
$projectMarkers = ['.git', 'index.html', 'currency.html', 'namaz.html'];
$repoRoot = findProjectRoot(__DIR__, $projectMarkers);
if (!@chdir($repoRoot)) {
    fwrite(STDERR, "WARNING: Unable to change directory to project root ({$repoRoot}). Using current directory instead.\n");
}

$xmlDir = 'currency-data';

if (!is_dir($xmlDir) && !@mkdir($xmlDir, 0775, true)) {
    fwrite(STDERR, "ERROR: Unable to create or access XML directory ({$xmlDir}).\n");
    exit(1);
}
...
for ($offset = 0; $offset < $searchWindow && count($retained) < $daysToKeep; $offset++) {
    ...
    $targetPath  = $xmlDir . DIRECTORY_SEPARATOR . $targetFile;
    ...
    if (file_exists($targetPath)) {
        ...
        if ($existing === $body) {
            $needsWrite = false;
            fwrite(STDOUT, "unchanged, already present in {$targetPath}.\n");
        }
    }

    if ($needsWrite) {
        file_put_contents($targetPath, $body);
        fwrite(STDOUT, "saved to {$targetPath}.\n");
    }
...
$retainedPaths = array_map(static function (string $filename) use ($xmlDir): string {
    return $xmlDir . DIRECTORY_SEPARATOR . $filename;
}, $retained);

fwrite(STDOUT, "\nRetained files: " . implode(', ', $retainedPaths) . "\n");
...
foreach ($allXml as $file) {
    ...
    if (!in_array($basename, $retained, true)) {
        @unlink($file);
